...
class Order_Status_Controller extends Controller
{
    ...
    {
$order_number=$id;
       return view('Product-Order-Screens.print')->with(array('order_number'=>$order_number));
    }

    public function print(Request $request, $id)
    {

        if($request->pel==null&&$request->pnn==null&&$request->print==null
            ||$request->pel==null&&$request->pnn==null||$request->print==null) {
            return redirect()->back();

        }
        else {
            $Order_print= Order::query()->where('id',$id)->update(['print'=>$request->print]);

            if($request->pel==null||$request->pnn==null){
                $Order_print= Order::find($id);
                $Order= Order::query()->where('id',$id)->
                update(['Amount'=>$Order_print->Amount+10]);
                ...
                $Order_print= Order::find($id);
                $Order= Order::query()->where('id',$id)->
                update(['Amount'=>$Order_print->Amount+20]);
            }
        	               ...
        	               '.$Delivery_Address.'</p>
        	                ...
        	                $emailcontent=array(
        	                    ...
        	                    ($loginid, $name,$id)
        	                    {
        	                        ...
        	                        $message->from('CLUBS_STORE@gmail.com','CLUBS STORE');
